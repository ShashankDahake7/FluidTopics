...
// ---------------------------------------------------------------------------
// Reader. Used by both /dashboard/docs/[id] and /r/[...path]:
//
//   documentId      — Mongo _id of the document to render (required)
//   initialTopicId  — pre-selected topic id from the resolver (optional)
//   allDocsHref     — back-link target ("/dashboard" by default)
// ---------------------------------------------------------------------------
export default function DocumentReader({ documentId, initialTopicId = null, allDocsHref = '/dashboard' }) {
  const id = documentId;
  ...
  useEffect(() => {
    ...
    api.get(`/portal/documents/${id}`)
      ...
      .then((data) => {
        ...
        //   1) initialTopicId prop (used by the pretty-URL resolver)
        //   2) ?topic= query param (legacy share links)
        //   3) first topic
  ...
  }, [selectedId, bookmarked, bookmarkBusy, t]);
  ...
  const handleContentClick = useCallback((e) => {
    ...
    // Already-rewritten dashboard/portal/pretty-URL link forms. We
    // recognise:
    //   /dashboard/docs/<topicId>[#hash]   (legacy / current)
    //   /portal/docs/<topicId>[#hash]      (older)
    //   /r/<...path>[#hash]                (pretty URL)
    // For /r/ links we let the browser navigate normally so the resolver
    // picks the right doc + topic; for the other two we know the topic id
    // is already in the URL and can swap content in-place.
    ...
    if (/^\/r\//.test(href)) {
      // Let Next.js handle the navigation — the resolver page will
      // re-instantiate this component with the right documentId.
      return;
    }
  ...
  const authorFromTocTopic = useMemo(() => {
    const tt = topics.find((x) => String(x._id) === String(selectedId));
    return tt?.metadata?.author || '';
  }, [topics, selectedId]);
  ...
  useEffect(() => {
    ...
    const isSectionHeader = (el) => {
      const tt = (el.textContent || '').trim().replace(/[:\s]+$/, '').toLowerCase();
      return tt === 'in this section';
    };
  ...
  if (!doc) {
    return (
      <div style={layout.notFound}>
        ...
        <Link href={allDocsHref} style={{ color: '#1d4ed8' }}>{t('backToPortal')}</Link>
      </div>
    );
  }
    ...
    <div style={layout.shell}>
      <aside style={layout.sidebar}>
        <div style={layout.sidebarHeader}>
          <Link href={allDocsHref} style={layout.backLink}>{t('allDocs')}</Link>
...
const shareStyles = {
  ...
  title: { fontSize: '1.05rem', fontWeight: 700, color: '#0f172a' },
  closeIcon: {
  ...
  body: { padding: '20px' },
  lead: { fontSize: '0.875rem', color: '#1f2937', marginBottom: '12px' },
  urlRow: {
...
const layout = {
  ...
  avatarPhoto: { flexShrink: 0, width: '36px', height: '36px' },
  avatarImage: {
